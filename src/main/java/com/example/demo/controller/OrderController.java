...
Long userId = (Long) body.get("user_id"); if (userId == null) { throw new NullPointerException("User ID is required"); }
    ...
    public List<Order> getUserOrders(@PathVariable Long userId) { return orderService.getOrdersByUser(userId); }
    @PostMapping("/{id}/cancel")
    public ResponseEntity<Order> cancelOrder(@PathVariable Long id) { return ResponseEntity.ok(orderService.cancelOrder(id)); }
    ...
    public Map<String, Object> getStats(@PathVariable Long userId) { return orderService.getOrderStats(userId); }
}
