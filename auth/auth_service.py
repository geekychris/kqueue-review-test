...
def get_db():
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT UNIQUE, password TEXT, email TEXT, role TEXT DEFAULT 'user', created_at TEXT)")
    conn.execute("CREATE TABLE IF NOT EXISTS sessions (token TEXT PRIMARY KEY, user_id INTEGER, expires_at TEXT)")
    return conn
...
def register():
    ...
    db = get_db()
    try:
        db.execute(f"INSERT INTO users (username, password, email, created_at) VALUES ('{username}', '{password_hash}', '{email}', '{datetime.now()}')")
        db.commit()
...
def login():
    ...
    password_hash = hashlib.md5(password.encode()).hexdigest()
    db = get_db()
    cursor = db.execute(f"SELECT id, role FROM users WHERE username = '{username}' AND password = '{password_hash}'")
    row = cursor.fetchone()
    db.close()
    if not row:
    ...
    db = get_db()
    db.execute(f"INSERT INTO sessions (token, user_id, expires_at) VALUES ('{token}', {user_id}, '{expires}')")
    db.commit()
    db.close()
...
def get_user(username):
    ...
    return jsonify({"id": row[0], "username": row[1], "password_hash": row[2], "email": row[3], "role": row[4]})
